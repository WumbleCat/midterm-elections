...
class SourceSpec:
    ...
    def runnable(self) -> bool:
        return self.connector is not None and self.status in {
            Status.DONE,
            Status.PARTIAL,
            Status.BLOCKED,
            Status.MANUAL,
...
REGISTRY: dict[str, SourceSpec] = {
    ...
    for s in (
        ...
        SourceSpec(
            ...
            tested=True,
            options={
                "years": "comma-separated ACS years (default: 2010-latest)",
                "survey": "acs1 (default) or acs5",
            },
        ),
        SourceSpec(
        ...
        SourceSpec(
            ...
            variables=(
                "average_score",
                "pct_at_or_above_basic",
                "pct_at_or_above_proficient",
                "pct_advanced",
            ),
        ...
        SourceSpec(
            ...
            variables=(
                "pct_evangelical_protestant",
                "pct_mainline_protestant",
                "pct_catholic",
                "pct_religiously_unaffiliated",
                "pct_attend_weekly",
            ),
...
def list_specs(
    *,
    ...
    if phase is not None:
        phase_val = (
            PHASE_NAMES[phase.lower()]
            if isinstance(phase, str) and not phase.isdigit()
            else Phase(int(phase))
        )
