...
def _window(
    df: pd.DataFrame, as_of: DateLike, days: int, date_col: str = "end_date"
) -> pd.DataFrame:
...
def _weighted_mean(values: pd.Series, weights: pd.Series | None) -> float | None:
    ...
    if weights is None or weights.isna().all():
        return float(v.mean())
    w = pd.to_numeric(weights, errors="coerce").fillna(
        v.dropna().shape[0] and pd.to_numeric(weights, errors="coerce").median() or 1.0
    )
...
def approval_features(approval: pd.DataFrame, as_of: DateLike) -> dict[str, float | None]:
    ...
    for days in (7, 30, 90):
        ...
        out[f"approval_average_{days}d"] = (
            _weighted_mean(w["approve"], w.get("sample_size")) if not w.empty else None
        )
        out[f"net_approval_{days}d"] = (
...
def generic_ballot_features(generic: pd.DataFrame, as_of: DateLike) -> dict[str, float | None]:
    ...
    for days in (7, 30, 90):
        ...
        out[f"generic_margin_{days}d"] = (
            _weighted_mean(w["generic_margin"], w.get("sample_size")) if not w.empty else None
        )
    w30 = _window(generic, as_of, 30)
    out["generic_ballot_dem_30d"] = (
        _weighted_mean(w30["generic_dem"], w30.get("sample_size")) if not w30.empty else None
    )
    out["generic_ballot_rep_30d"] = (
...
def race_poll_features(
    polls: pd.DataFrame,
    year: int,
    office: str,
    as_of: DateLike,
    election_date: DateLike | None = None,
) -> pd.DataFrame:
    ...
    for k, g in filter_as_of(df, as_of).groupby(key):
        ...
        rec["polling_average_7d"] = (
            _weighted_mean(g7["poll_margin"], g7.get("sample_size")) if not g7.empty else None
        )
        rec["polling_average_30d"] = (
            _weighted_mean(g30["poll_margin"], g30.get("sample_size")) if not g30.empty else None
        )
        ...
        rec["days_to_election"] = (
            int((pd.Timestamp(to_date(election_date)) - cutoff).days)
...
def special_election_swing(specials: pd.DataFrame, as_of: DateLike) -> dict[str, float | None]:
    ...
    if specials is None or specials.empty:
        return {
            "special_election_swing_30d": None,
            "special_election_swing_90d": None,
            "special_election_swing_180d": None,
        }
    for days in (30, 90, 180):
        w = _window(specials, as_of, days, date_col="election_date")
        out[f"special_election_swing_{days}d"] = (
            float(w["special_election_swing"].mean()) if not w.empty else None
        )
    return out
