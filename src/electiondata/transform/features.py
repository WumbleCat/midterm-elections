...
from ..quality.point_in_time import DateLike, assert_no_future_rows, filter_as_of, latest_as_of, to_date
...
def _provenance(df: pd.DataFrame, as_of: DateLike | None = None) -> dict[str, Any]:
    ...
    info: dict[str, Any] = {
        ...
        "ingestion_runs": sorted(df["ingestion_run_id"].dropna().unique().tolist())[:20] if "ingestion_run_id" in df else [],
    }
    if "publication_date" in df:
        ...
        if "publication_date_estimated" in df:
            info["publication_date_estimated_share"] = round(float(df["publication_date_estimated"].fillna(False).mean()), 3)
...
def _race_universe(office: str, year: int, summary: pd.DataFrame, candidates: pd.DataFrame, as_of: DateLike) -> pd.DataFrame:
    """The set of races to build rows for."""
    key = _race_key(office)
    target = summary[(summary["year"] == year) & (summary["office"] == office) & (~summary["special"].fillna(False))]
    if not target.empty:
    ...
    log.warning("no results or candidates for race universe; using all states", extra={"office": office, "year": year})
    return pd.DataFrame({"state": list(FIFTY_STATES)}) if office != "house" else pd.DataFrame(columns=key)
...
def build_features(
    ...
    *,
    ...
    if as_of_date > election_date:
        log.warning("as_of is after election day: the build is a post-election snapshot, not a forecast", extra={"as_of": str(as_of_date)})
    key = _race_key(office)
    ...
    if results.empty:
        raise DatasetUnavailableError("election_results is empty; ingest medsl-senate/medsl-president first")
    ...
    frame["state_fips"] = frame["state"].map(lambda s: STATE_BY_ABBR[s].fips if s in STATE_BY_ABBR else None)
    ...
    hist_input = pd.concat([avail_summary[avail_summary["year"] < year], stub], ignore_index=True, sort=False)
    hist = elec_t.political_history(hist_input, office)
    hist = hist[hist["year"] == year].drop(columns=["year", "office", "election_date"], errors="ignore")
    if office != "house" and "district" in hist.columns:
        hist = hist.drop(columns=["district"])
    frame = frame.merge(hist, on=key, how="left")
    families["political_history"] = _provenance(avail_summary[avail_summary["year"] < year], as_of_date)
    ...
    if not turnout.empty:
        ...
        if not metrics.empty:
            ...
            tcols_turn = ["state", "year", "turnout_registered", "registered_voters", "ballots_cast"] + [c for c in ("turnout_population", "turnout_cvap", "mail_vote_share", "early_vote_share") if c in last.columns]
            last = last[tcols_turn].rename(columns={c: (f"prev_{c}" if c != "year" else "prev_turnout_year") for c in tcols_turn if c != "state"})
    ...
    d = demo_t.demographics_as_of(demographics, as_of_date) if not demographics.empty else pd.DataFrame()
    if not d.empty:
        frame = frame.merge(d, on="state", how="left")
    families["demographics"] = _provenance(filter_as_of(demographics, as_of_date) if not demographics.empty else demographics, as_of_date)
    ...
    families["population"] = _provenance(filter_as_of(population, as_of_date) if not population.empty else population, as_of_date)
    families["geography"] = _provenance(filter_as_of(geography, as_of_date) if not geography.empty else geography, as_of_date)
    ...
    families["urban_rural"] = _provenance(filter_as_of(urban, as_of_date) if not urban.empty else urban, as_of_date)
    ...
    families["labor"] = _provenance(filter_as_of(labor, as_of_date) if not labor.empty else labor, as_of_date)
    ...
    families["industry"] = _provenance(filter_as_of(industry, as_of_date) if not industry.empty else industry, as_of_date)
    ...
    families["state_economy"] = _provenance(filter_as_of(econ, as_of_date) if not econ.empty else econ, as_of_date)
    ...
    if not national.empty:
        ...
        for k, v in nat.items():
            frame[k] = v
    families["national_economy"] = _provenance(filter_as_of(national, as_of_date) if not national.empty else national, as_of_date)
    ...
    families["incumbency"] = _provenance(filter_as_of(candidates, as_of_date, keep_unknown=True) if not candidates.empty else candidates, as_of_date)
    ...
    if not finance.empty:
        ...
        usable = cyc[pd.to_datetime(cyc["retrieval_date"], errors="coerce") <= pd.Timestamp(as_of_date)]
        fam = _provenance(filter_as_of(usable, as_of_date), as_of_date)
        if fam["rows"] == 0:
            fam["note"] = "no finance snapshot retrieved on/before as_of (bulk summaries are post-hoc snapshots); see fec-committee-reports in ROADMAP"
    else:
    ...
    families["race_polls"] = _provenance(filter_as_of(polls, as_of_date) if not polls.empty else polls, as_of_date)

    approval = _load("approval_polls", paths)
    if not approval.empty:
        for k, v in polls_t.approval_features(approval, as_of_date).items():
            frame[k] = v
    families["approval"] = _provenance(filter_as_of(approval, as_of_date) if not approval.empty else approval, as_of_date)
    ...
    if not generic.empty:
        for k, v in polls_t.generic_ballot_features(generic, as_of_date).items():
            frame[k] = v
    families["generic_ballot"] = _provenance(filter_as_of(generic, as_of_date) if not generic.empty else generic, as_of_date)
    ...
    if not specials.empty:
        for k, v in polls_t.special_election_swing(specials, as_of_date).items():
            frame[k] = v
    families["special_elections"] = _provenance(filter_as_of(specials, as_of_date) if not specials.empty else specials, as_of_date)

    # ------------------------------------------------------ supplementary
    naep = _load("naep", paths)
    if not naep.empty:
        n = latest_as_of(naep, as_of_date, keys=["state", "grade", "subject"], order_col="assessment_year")
        if not n.empty:
            n["col"] = "naep_" + n["subject"].astype(str).str[:4] + "_grade" + n["grade"].astype(str)
            wide = n.pivot_table(index="state", columns="col", values="average_score", aggfunc="first").reset_index()
            wide.columns.name = None
            wide["naep_year"] = n.groupby("state")["assessment_year"].max().reindex(wide["state"]).values
            frame = frame.merge(wide, on="state", how="left")
    families["naep"] = _provenance(filter_as_of(naep, as_of_date) if not naep.empty else naep, as_of_date)
    ...
    if not religion.empty:
        ...
        if not r.empty:
            ...
            frame = frame.merge(r[rcols].rename(columns={"survey_year": "religion_survey_year"}), on="state", how="left")
    families["religion"] = _provenance(filter_as_of(religion, as_of_date) if not religion.empty else religion, as_of_date)
    ...
    build = FeatureBuild(frame=frame, year=year, office=office, as_of=as_of_date, election_date=election_date, metadata=metadata)
    if write:
        ...
        (paths.features / f"{stem}.json").write_text(json.dumps(metadata, indent=2, default=str), encoding="utf-8")
        log.info("features written", extra={"path": str(build.path), "rows": len(frame), "columns": frame.shape[1]})
    return build
...
def _verify_no_leakage(families: dict[str, dict[str, Any]], as_of: dt.date) -> None:
    ...
    for name, info in families.items():
        ...
        if mx and dt.date.fromisoformat(mx) > as_of:
            raise RuntimeError(f"family {name} contains rows published after as_of ({mx} > {as_of})")
...
def audit_features(build: FeatureBuild) -> dict[str, Any]:
    ...
    return {
        ...
        "families": {k: {"available": v.get("available"), "max_publication_date": v.get("max_publication_date"), "estimated_share": v.get("publication_date_estimated_share")} for k, v in fam.items()},
        "post_as_of_families": [k for k, v in fam.items() if k != "targets" and v.get("max_publication_date") and dt.date.fromisoformat(v["max_publication_date"]) > build.as_of],
    }


__all__ = ["FAMILIES", "FeatureBuild", "assert_no_future_rows", "audit_features", "build_features", "load_features"]
