...
def demographics_as_of(demographics: pd.DataFrame, as_of: DateLike, *, survey: str = "acs1") -> pd.DataFrame:
    """Latest ACS profile per state published on or before ``as_of``."""
    df = demographics[demographics["survey"] == survey] if "survey" in demographics.columns else demographics
    latest = latest_as_of(df, as_of, keys=["state"], order_col="year")
    if latest.empty:
        return latest
    cols = ["state", "year", "publication_date", *[c for c in DEMOGRAPHIC_FEATURES if c in latest.columns]]
    out = latest[cols].rename(columns={"year": "acs_year", "publication_date": "acs_publication_date"})
...
def population_as_of(population: pd.DataFrame, as_of: DateLike) -> pd.DataFrame:
    ...
    if avail.empty:
        return pd.DataFrame(columns=["state", "population", "population_year", "population_growth_1y", "population_growth_4y", "population_growth_10y"])
    avail = avail.copy()
    ...
    series = avail.sort_values(["state", "year", "_v"]).groupby(["state", "year"], as_index=False).tail(1)
    ...
    out = latest[["state", "year", "population", "population_growth_1y", "population_growth_4y", "population_growth_10y", "domestic_migration", "international_migration", "net_migration", "domestic_migration_rate", "net_migration_rate", "publication_date", "revision_vintage"]].rename(
        columns={"year": "population_year", "publication_date": "population_publication_date", "revision_vintage": "population_vintage"}
    )
    return out.reset_index(drop=True)


def density_features(pop_features: pd.DataFrame, geography: pd.DataFrame, as_of: DateLike) -> pd.DataFrame:
...
def urban_features(urban_rural: pd.DataFrame, as_of: DateLike) -> pd.DataFrame:
    ...
    return latest[["state", "pct_urban", "pct_rural", "year"]].rename(columns={"year": "urban_rural_census_year"}).reset_index(drop=True)
